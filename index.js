...
waktu = '-'
alasan = '-'
join = '\`\`\`Hi 👋\`\`\` \n \`\`\`New Member Intro Dulu!\`\`\` \n \`\`\`Nama :\`\`\` \n \`\`\`Askot : \`\`\` \n \`\`\`Umur :\`\`\` \n \`\`\`Status :\`\`\` \n\n'
leave = '\`\`\`Sayonaraa 👋 Semoga kita bisa bertemu lagi\`\`\`'
//=================================================//
...
//========================================================================================================================//
		...
		const isQuotedSticker = type === 'extendedTextMessage' && content.includes('stickerMessage')
     	...
     	if (isCmd && isGroup) console.log('\[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(command), 'from', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
...
*</MAKER>*
► _${prefix}sticker_
► _${prefix}stickerwm_ <author|packname>
...
❏ *SELF-BOT* ❏`
    ...
    case 'fitnahpc':
            ...
            case 'grup':
					...
					break
					case 'welcome':
					if (!mek.key.fromMe) return reply('*Perintah ini hanya bisa di gunakan oleh owner bot!*')
					if (!isGroup) return reply(mess.only.group)
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Fitur tersebut sudah aktif pada grup ini')
						welkom.push(from)
				...
				case 'cekchat':
					...
					hexa.sendMessage(from, `Total : ${totalchat.length} Chat`, text, {quoted  : mek})
					break
            case 'getpic':
				if (mek.message.extendedTextMessage != undefined){
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					hexa.sendMessage(from, thumb, MessageType.image)
				{quoted : mek}}
				break
				case 'getbio':
                ...
                break
                case 'wikipedia':
                    if (args.length < 0) return reply(`Contoh: ${prefix}wikipedia Puasa`)
                    query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/wiki?apikey=4286921bc9caa1a76b14ad0a&query=${query}`)
                    get_result = get_result.result
                    hexa.sendMessege(from, get_result, text, { quoted: mek })
                    break
				case 'asmaulhusna':
				...
				case'rimage':
					...
					hexa.sendMessage(from, nye, image, { caption: 'NIH', quoted: mek })
					break
				case 'tomp3':
    ...
    case 'flip':
	    ...
	    hexa.sendMessage(from, ip, text, { quoted: mek })
            break
    case 'dice':
            dice = body.slice(1)
            const di =['1','2','3','4','5','6']
            const ce = di[Math.floor(Math.random() * di.length)]
            hexa.sendMessage(from, ce, text, { quoted: mek })
            break
    case 'fast':
            if (!isQuotedVideo) return fakegroup('Reply videonya!')
            fakegroup(mess.wait)
    ...
    case 'ssweb':
            kontol = args.join(" ")
            memek = await getBuffer(`https://shot.screenshotapi.net/screenshot?&url=${kontol}&output=image&file_type=png&wait_for_event=load`)
            hexa.sendMessage(from, memek, image,{quoted: mek,caption:'NIH'})
            break
    case 'slow':
            if (!isQuotedVideo) return fakegroup('Reply videonya!')
            fakegroup(mess.wait)
    ...
    case 'gay':
    	    ...
    	    hexa.sendMessage(from, 'Tingkat kecocokan '+gay+'\n\nAdalah : '+ ay, text, { quoted: mek })
    	    break
    case 'kontak':
            pe = args.join(' ') 
            entah = pe.split('|')[0]
	...
	case 'self':
          ...
          case 'promote':
                   ...
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (hexa.message.extendedTextMessage === undefined || hexa.message.extendedTextMessage === null) return reply('Tag target yang ingin di jadi admin!')
					mentioned = hexa.message.extendedTextMessage.contextInfo.mentionedJid
 	...
 	case 'hidetag':
			...
			break
case 'tagall':
if (!mek.key.fromMe) return reply(```Hanya Untuk Saya```)
teks = '*[Mention All]*\n\n'+'```INFO: '+q+'```\n\n\n'
n = 1
yoi = [' ']
...
hexa.sendMessage(from, teks, text,{contextInfo: {mentionedJid: groupMembers.map(me => me.jid)}})
break
          case 'leave': 
                    if (!mek.key.fromMe) return reply('```Hanya Untuk Saya```')
				    if (!isGroup) return reply(mess.only.group)
	...
	case 'ytmp3':
			...
			if (!isLinks) return reply(mess.error.Iv)
			    ...
			    hexa.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
		            break
    case 'image':
    ...
    case 'fb':
            ...
            break    
	case '>':
	                if (!mek.key.fromMe) return reply('```Hanya Oleh Saya```')
			if (!q) return fakegroup(mess.wrongFormat)
			exec(q, (err, stdout) => {
...
if(budy.includes('ERIK')){
reply('Ya,knp?')
}
if(budy.includes('elik')){
reply('Ya,knp?')
}
if(budy.includes('Erik')){
reply('Ya,knp?')
